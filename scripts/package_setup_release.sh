...
#
# Writes: dist/megamanlegends2-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix megamanlegends2 \
  --exe-name Mega_Man_Legends_2 \
  --omit-openbios \
  --display-name "Mega Man Legends 2" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Mega Man Legends 2 (Europe) disc" \
  --runtime-dir mods \
